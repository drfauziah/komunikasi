...
try:
    while status == 1:
        ...
        connection_bms = pymysql.connect(host='localhost',database='bms',user='root',password='')
        ...
        if time_len_server == 0:
            ...
            for row in cursor.fetchall():
                ...
                #Memberi syntax SQL untuk memasukkan data channel, timestamp, entry_id dan nilai sensor - sensor dari tabel tabeldata
                query="insert into feeds (channel, created_at, entry_id, field1, field2, field3, field4, field5, field6) values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');"%(channel, datetime, entry_id, datasatu, datadua, datatiga, dataempat, datalima, dataenam)
                cursor.execute(query)
                connection_bms.commit()
                entry_id=entry_id+1
